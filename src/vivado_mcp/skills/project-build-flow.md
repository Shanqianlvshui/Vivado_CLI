...
9. For testbench work, use `vivado_simulation_audit`, `vivado_prepare_simulation(dry_run=true)` before non-trivial fileset changes, `vivado_launch_simulation`, and `vivado_analyze_xsim_logs`.
10. For Non-project Mode, use `vivado_nonproject_audit`, dry-run `vivado_nonproject_read_sources` or step tools for non-trivial flows, then run `vivado_nonproject_synth_design`, `vivado_nonproject_opt_design`, `vivado_nonproject_place_design`, and `vivado_nonproject_route_design`.
