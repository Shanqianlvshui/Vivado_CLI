...
5. Call `vivado_bd_apply(dry_run=true)` before nontrivial generic actions such as `create_cell`, `create_port`, `connect_net`, `set_property`, `apply_automation`, `assign_address`, `validate`, or `save`.
6. Call `vivado_bd_apply` for the accepted plan, then `vivado_bd_validate`.
