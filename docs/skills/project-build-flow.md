...
9. For testbench work, call `vivado_simulation_audit`, then `vivado_prepare_simulation(dry_run=true)` before non-trivial fileset changes, then `vivado_launch_simulation` and `vivado_analyze_xsim_logs` before changing RTL or IP based on a simulator failure.
10. For Non-project Mode, call `vivado_nonproject_audit`, dry-run `vivado_nonproject_read_sources` or step tools for non-trivial flows, then `vivado_nonproject_synth_design`, `vivado_nonproject_opt_design`, `vivado_nonproject_place_design`, and `vivado_nonproject_route_design`.
