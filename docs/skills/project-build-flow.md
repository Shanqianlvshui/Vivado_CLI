...
6. For complex projects, call `vivado_source_audit` first, then use `vivado_list_filesets` and `vivado_describe_fileset` to understand the active filesets, top modules, and included files before adding anything new.
7. For non-trivial changes, call `vivado_capture_state` first or pass `capture_diff=true` to the mutating tool.
8. Call `vivado_add_sources` for RTL and XDC files. Use the `sources_fileset`, `include_dirs`, `defines`, `library`, `file_type`, `used_in`, and `processing_order` parameters when the project is not the default `sources_1`.
9. For project IP, call `vivado_ip_catalog_search`, then `vivado_create_ip(dry_run=true)` for the plan before creation on non-trivial IP, then `vivado_describe_ip`, `vivado_ip_upgrade_check`, and `vivado_generate_ip_outputs`. Use `vivado_upgrade_ip(expect_upgrade=true)` only when the `.xci` mutation is intended.
10. For testbench work, call `vivado_simulation_audit`, then `vivado_prepare_simulation(dry_run=true)` before non-trivial fileset changes, then `vivado_launch_simulation` and `vivado_analyze_xsim_logs` before changing RTL or IP based on a simulator failure.
11. For Non-project Mode, call `vivado_nonproject_audit`, dry-run `vivado_nonproject_read_sources` or step tools for non-trivial flows, then `vivado_nonproject_synth_design`, `vivado_nonproject_opt_design`, `vivado_nonproject_place_design`, and `vivado_nonproject_route_design`.
12. For read-only hardware discovery, call `vivado_hw_discover(expect_hardware_access=true)`; do not program devices through structured tools.
13. Call `vivado_set_top` (with `top=None` to read, or pass a value to set) to confirm or change the top module.
14. Call `vivado_set_file_properties` for files that need explicit LIBRARY / PROCESSING_ORDER / USED_IN overrides.
15. Call `vivado_xdc_order_check` and `vivado_constraint_diagnostics` to audit XDC fileset ordering, USED_IN scopes, and UG903/UG949 methodology markers before synthesis on non-trivial projects.
16. Call `vivado_run_synthesis`.
17. If synthesis succeeds, call `vivado_run_implementation`.
18. Call `vivado_analyze_reports` for `timing_summary`, `timing_paths`, `clock_interaction`, `utilization`, `drc`, `power`, and `methodology`; use `quality_gates`, `next_action_plan`, issue IDs, evidence, root-cause hints, and `official_doc_queries` to choose the next action.
19. Use targeted `vivado_report` calls only after the aggregate analysis points at the next failure area.
20. Summarize WNS/TNS, utilization pressure, DRC/methodology rule IDs, power totals, and the first actionable errors.
...
- `simulation_compile_or_elaboration_failed`: call `vivado_analyze_xsim_logs` and `vivado_simulation_audit`, then inspect the simulation fileset with `vivado_describe_fileset`.
- `nonproject_step_failed`: inspect `vivado_nonproject_audit`, the command artifact, prerequisites, and requested reports from the failed Non-project step.
