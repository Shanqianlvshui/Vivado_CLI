...
10. Call `vivado_set_top` (with `top=None` to read, or pass a value to set) to confirm or change the top module.
11. Call `vivado_set_file_properties` for files that need explicit LIBRARY / PROCESSING_ORDER / USED_IN overrides.
12. Call `vivado_xdc_order_check` and `vivado_constraint_diagnostics` to audit XDC fileset ordering, USED_IN scopes, and UG903/UG949 methodology markers before synthesis on non-trivial projects.
13. Call `vivado_run_synthesis`.
14. If synthesis succeeds, call `vivado_run_implementation`.
15. Call `vivado_analyze_reports` for `timing_summary`, `utilization`, `drc`, `power`, and `methodology`.
16. Use targeted `vivado_report` calls only after the aggregate analysis points at the next failure area.
17. Summarize WNS/TNS, utilization pressure, DRC/methodology rule IDs, power totals, and the first actionable errors.
