...
4. Call `vivado_add_sources` for RTL, IP, and XDC files.
5. Call `vivado_run_synthesis`.
6. If synthesis succeeds, call `vivado_run_implementation`.
7. Call `vivado_report` for `timing_summary`, `utilization`, `drc`, and `messages`.
8. Summarize WNS/TNS, utilization, critical warnings, and the first actionable errors.
