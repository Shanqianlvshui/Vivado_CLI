...
Official-document guidance is exposed as metadata, topic routing, and local PDF search, not copied AMD document bodies. Use `vivado_official_reference_guide`, `vivado_search_official_docs`, `vivado_list_official_references`, and `vivado_get_official_reference` to find the relevant AMD source before generating Tcl or using expert mode.
